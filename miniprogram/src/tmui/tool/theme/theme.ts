...
class themeColors {
	...
	public getTheme(config: cssStyleConfig = { colorname: 'primary',dark:false }): cssstyle {
		...
		if (nowColor.hsla.h == 0 && nowColor.hsla.s == 0 && config.dark) {
			css.backgroundColor = colortool.rgbaToCss(colortool.hslaToRgba({ ...bghsl, l: 9 }));
			css.border = colortool.rgbaToCss(colortool.hslaToRgba({ ...borderhsl, l: 16 }));
		}
		...
		if (config.linearDirection) {
			...
			if (nowColor.hsla.h == 0 && nowColor.hsla.s == 0 && nowColor.hsla.l == 100) {
				...
				if (config.linearDeep == 'light') {
					...
					liner_color_1.h = liner_color_1.h - 5;//色相需要往前偏移加强色系
					liner_color_1.s = liner_color_1.s + 20;//饱和度需要加强
					liner_color_1.l = 65;
					liner_color_2.l = 65;
					liner_color_2.h = liner_color_2.h + 30;//偏移30度的色相，相当于35度的搭配色进行渐变
					liner_color_2.s = liner_color_2.s + 20;//同样饱和度加强20
				}
		...
		if (config.dark == true) {
			css.cardcolor = 'rgba(20, 20,20, 1.0)'; //项目
			css.inputcolor = '#1f1f1f';//输入框，表单等
		...
		if(config.dark){
			...
			if(nowColor.hsla.h == 0 && nowColor.hsla.s == 0){
				css.border = colortool.rgbaToCss(colortool.hslaToRgba({ ...nowColor.hsla, l: 20 }));
			}
		}else{
			if(nowColor.hsla.h != 0 && nowColor.hsla.s != 0){
				css.border = colortool.rgbaToCss(colortool.hslaToRgba({ ...nowColor.hsla, l: bghsl.l-10 }));
